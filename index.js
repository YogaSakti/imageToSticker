...
const start = (client = new Client()) => {
    ...
    client.onAddedToGroup(({ groupMetadata: { id }, contact: { name } }) =>
        client.getGroupMembersId(id)
            .then((ids) => {
                ...
                if (ids.length <= 10) {
                    client.sendText(id, 'Maaf, minimal orang untuk menggunakan bot ini adalah 10 orang. Bye~').then(() => client.leaveGroup(id))
                } else {
                    client.sendText(id, `Halo member grup *${name}*, terimakasih telah mengundang bot, gunakan *#menu* untuk melihat perintah`)
                }
            }))
    ...
    client.onGlobalParicipantsChanged((event) => {
        // if (event.action === 'add') client.sendTextWithMentions(event.chat, `Halo, Selamat datang di grup @${event.who.replace('@c.us', '')} \n\nHave fun with us✨`)
    })
