...
exports.fetchMeme = fetchMeme = () => new Promise((resolve, reject) => {
    const subreddit = ['dankmemes', 'wholesomeanimemes', 'wholesomememes', 'AdviceAnimals', 'MemeEconomy', 'memes', 'terriblefacebookmemes', 'historymemes']
...
exports.fetchWeebs = fetchWeebs = () => new Promise((resolve, reject) => {
    const subreddit = ['BanGDream', 'Granblue_en', 'arknights']
    const randSub = subreddit[Math.random() * subreddit.length | 0]
    console.log('looking for memes on ' + randSub)
    fetch('https://meme-api.herokuapp.com/gimme/' + randSub)
        .then(response => response.json())
